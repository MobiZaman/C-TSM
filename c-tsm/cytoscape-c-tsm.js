(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["cytoscapeCTsm"] = factory(require("cose-base"), require("layout-base"));
	else
		root["cytoscapeCTsm"] = factory(root["coseBase"], root["layoutBase"]);
...
var register = function register(cytoscape) {
  ...
  cytoscape('layout', 'c-tsm', impl); // register with cytoscape.js
};
...
var ctsmLayout = __webpack_require__(3);
...
var Layout = function () {
  ...
  _createClass(Layout, [{
    ...
    value: function run() {
      ...
      var coseLayout = layout.runCoseLayout(options, coseIdToLNode, this.ctsmNodeToCoseNode, topMostNodes);
      options.randomize = false;

      //Reflect changes back to ctsm nodes
      var ctsmNodes = gm.getAllNodes();
      for (var i = 0; i < ctsmNodes.length; i++) {
        var ctsmNode = ctsmNodes[i];
        var coseNode = this.ctsmNodeToCoseNode[ctsmNode.id];
      ...
      fetch('/tsm/', {
        ...
        var ctsmNodes = gm.getAllNodes();
        for (var _i3 = 0; _i3 < ctsmNodes.length; _i3++) {
          var _ctsmNode = ctsmNodes[_i3];
          if (_ctsmNode.isCompound()) continue;

          var newPos = nodes[_ctsmNode.id];
          _ctsmNode.setCenter(newPos[0], newPos[1]);
        }
        ...
        if (oldGm != undefined) {
          ...
          for (var _i4 = 0; _i4 < ctsmNodes.length; _i4++) {
            var _ctsmNode2 = ctsmNodes[_i4];

            if (_ctsmNode2.isCompound()) continue;

            var _newPos = nodeDict[_ctsmNode2.id].getCenter();
            _ctsmNode2.setCenter(_newPos.x, _newPos.y);
          }
          gm.removeConnectivityEdges();
        }

        if (options.compact) layout.compactGraph(gm, options.maxPasses, options.idealEdgeLength);

        if (oldGm != undefined) {
          //Reflect changes back to actual ctsm nodes
          ctsmNodes = oldGm.getAllNodes();
          for (var _i5 = 0; _i5 < ctsmNodes.length; _i5++) {
            var _ctsmNode3 = ctsmNodes[_i5];

            if (_ctsmNode3.isCompound()) continue;

            var _newPos2 = nodeDict[_ctsmNode3.id].getCenter();
            _ctsmNode3.setCenter(_newPos2.x, _newPos2.y);
          }
        }
        ...
        var ctsmEdges2 = gm.edgesWithBends;
        for (var _i6 = 0; _i6 < ctsmEdges2.length; _i6++) {
          ...
          var ctsmEdge = ctsmEdges2[_i6];
          var cyEdge = cyEdgesMap[ctsmEdge.id];
          ...
          if (cyEdge != undefined) {
            ...
            if (oldGm != undefined) {
              var _edge = ctsmEdgesMap[ctsmEdge.id];
              copyBps(ctsmEdge, _edge);
            }
          } else {
            var source = ctsmEdge.source;
            var target = ctsmEdge.target;

            var _edge2 = void 0;
            if (source.isDummy && source.dummyOwner.isCompound() && !target.isDummy) {
              _edge2 = source.dummyOwner.findEdgeBetween(ctsmIdToLNode[target.id]);
            } else if (target.isDummy && target.dummyOwner.isCompound() && !source.isDummy) {
              _edge2 = target.dummyOwner.findEdgeBetween(ctsmIdToLNode[source.id]);
            ...
            copyBps(ctsmEdge, _edge2);

            _edge2.sourcePoint = ctsmEdge.source.getCenter();
            _edge2.targetPoint = ctsmEdge.target.getCenter();
          }
        }
...
var ctsmGraphManager = __webpack_require__(6);
var ctsmNode = __webpack_require__(7);
var ctsmEdge = __webpack_require__(8);
var ctsmGraph = __webpack_require__(9);
...
ctsmLayout.prototype.setParents = function (gm) {
  ...
  for (var i = 0; i < allNodes.length; i++) {
    var _ctsmNode = allNodes[i];
    if (_ctsmNode.owner.parent.id != undefined) {
      _ctsmNode.parentNode = _ctsmNode.owner.parent;
    }
  }
};
...
ctsmLayout.prototype.processEdges = function (layout, gm, edges, idToLNode, cyEdgesMap, ctsmEdgesMap) {
  ...
  for (var i = 0; i < edges.length; i++) {
    ...
    if (ctsmrun) cyEdgesMap[edge.id()] = edge;
    ...
    if (sourceNode !== targetNode) {
      if (sourceNode.getEdgesBetween(targetNode).length == 0) {
        ...
        if (ctsmEdgesMap != null) ctsmEdgesMap[e.id] = e;
      }
    }
...
ctsmLayout.prototype.convertToSimpleGraph = function (gm, compoundNodes) {
  ...
  for (var i = 0; i < allNodes.length; i++) {
    var node = allNodes[i];
    if (!node.isCompound()) {
      var newNode = parent.add(this.newNode(node.getLocation(), new DimensionD(node.getWidth(), node.getHeight())));
      newNode.id = node.id;
  ...
  for (var _i3 = 0; _i3 < compoundNodes.length; _i3++) {
    ...
    for (var j = 0; j < edges.length; j++) {
      ...
      var _newNode = parent.add(this.newNode({ x: endpoint1.x - 0.5, y: endpoint1.y - 0.5 }, new DimensionD(1, 1)));
...
ctsmLayout.prototype.createDummiesForCrossings = function (gm, edgeCrosses, edgeSplitDict, nodeDict) {
  ...
  for (var i = 0; i < edgeCrosses.length; i++) {
    ...
    var dummyNode = parent.add(this.newNode(crossingPoint, new DimensionD(1, 1)));
...
ctsmLayout.prototype.compactGraph = function (gm, maxPasses, iel) {
  ...
  for (var i = 1; i <= maxPasses; i++) {
    ...
    var area1 = void 0,
        ...
        area4 = void 0;
    if (i % 4 == 1) {
      var _compactGraphHelper = this.compactGraphHelper(gm, iel, "y", TOP_TO_BOTTOM);
      ...
      var _compactGraphHelper3 = this.compactGraphHelper(gm, iel, "x", LEFT_TO_RIGHT);
      ...
      var _compactGraphHelper5 = this.compactGraphHelper(gm, iel, "y", BOTTOM_TO_TOP);
      ...
      var _compactGraphHelper7 = this.compactGraphHelper(gm, iel, "x", RIGHT_TO_LEFT);
      ...
      var _compactGraphHelper9 = this.compactGraphHelper(gm, iel, "y", BOTTOM_TO_TOP);
      ...
      var _compactGraphHelper11 = this.compactGraphHelper(gm, iel, "x", RIGHT_TO_LEFT);
      ...
      var _compactGraphHelper13 = this.compactGraphHelper(gm, iel, "y", TOP_TO_BOTTOM);
      ...
      var _compactGraphHelper15 = this.compactGraphHelper(gm, iel, "x", LEFT_TO_RIGHT);
      ...
      var _compactGraphHelper17 = this.compactGraphHelper(gm, iel, "x", LEFT_TO_RIGHT);
      ...
      var _compactGraphHelper19 = this.compactGraphHelper(gm, iel, "y", TOP_TO_BOTTOM);
      ...
      var _compactGraphHelper21 = this.compactGraphHelper(gm, iel, "x", RIGHT_TO_LEFT);
      ...
      var _compactGraphHelper23 = this.compactGraphHelper(gm, iel, "y", BOTTOM_TO_TOP);
      ...
      var _compactGraphHelper25 = this.compactGraphHelper(gm, iel, "x", RIGHT_TO_LEFT);
      ...
      var _compactGraphHelper27 = this.compactGraphHelper(gm, iel, "y", BOTTOM_TO_TOP);
      ...
      var _compactGraphHelper29 = this.compactGraphHelper(gm, iel, "x", LEFT_TO_RIGHT);
      ...
      var _compactGraphHelper31 = this.compactGraphHelper(gm, iel, "y", TOP_TO_BOTTOM);
...
ctsmLayout.prototype.reLocateBarMembers = function (gm, indexDict, dir, iel, axis) {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    if (bar.newLoc == 0 && bar == refBar) continue;else {
      var newPos = refPos + bar.newLoc * iel;
      var nodes = bar.nodes;

      //change the node positions only if they are different than the new ones
      if (nodes[0] instanceof ctsmNode && (axis == "x" && newPos == nodes[0].getCenterX() || axis == "y" && newPos == nodes[0].getCenterY())) continue;else if (!(nodes[0] instanceof ctsmNode) && (axis == "x" && newPos == nodes[0].x || axis == "y" && newPos == nodes[0].y)) continue;
  ...
  if (dir == 0) {
    ...
    for (var i = bars.length - 1; i >= 0; i--) {
      ...
      if (futureBars.length == 0) {
        if (i == bars.length - 1) bar.newLoc = 0;else bar.newLoc = (parseInt(bar.dictKey) - parseInt(bars[bars.length - 1].dictKey)) / iel;
      } else {
    ...
    for (var _i6 = 0; _i6 < bars.length; _i6++) {
      ...
      if (prevBars.length == 0) {
        if (_i6 == 0) _bar.newLoc = 0;else _bar.newLoc = (parseInt(_bar.dictKey) - parseInt(bars[0].dictKey)) / iel;
      } else {
...
ctsmGraphManager.prototype.getAverageEdgeLength = function () {
  ...
  for (var i = 0; i < allEdges.length; i++) {
    ...
    if (bps.length == 0) {
      var n1Pos = edge.source.getCenter();
      var n2Pos = edge.target.getCenter();
      ...
      for (var j = 0; j <= bps.length; j++) {
        if (j == 0) {
          _n1Pos = edge.source.getCenter();
          _n2Pos = { x: bps[j].x, y: bps[j].y };
        } else if (j == bps.length) {
          _n1Pos = { x: bps[j - 1].x, y: bps[j - 1].y };
          _n2Pos = edge.target.getCenter();
        } else {
          _n1Pos = { x: bps[j - 1].x, y: bps[j - 1].y };
          _n2Pos = { x: bps[j].x, y: bps[j].y };
        }

        len = Math.sqrt(Math.pow(_n1Pos.x - _n2Pos.x, 2) + Math.pow(_n1Pos.y - _n2Pos.y, 2));
        sum = sum + len;
      }
    }
  }
  ...
  for (var _j = 0; _j < this.boundaryList.length; _j++) {
    ...
    if (edge != null && edge.bendpoints.length > 0) {
      ...
      for (var k = 0; k < bps.length; k++) {
        ...
        var newNode = parent.add(new ctsmNode(simpleGm, { x: bp.x, y: bp.y }, new DimensionD(1, 1)));
...
function ctsmGraph(parent, graphMgr, vGraph) {
    LGraph.call(this, parent, graphMgr, vGraph);
}

ctsmGraph.prototype = Object.create(LGraph.prototype);

for (var prop in LGraph) {
    ctsmGraph[prop] = LGraph[prop];
};

ctsmGraph.prototype.updateBounds = function (recursive) {
    // calculate bounds
